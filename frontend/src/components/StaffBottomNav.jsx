/**
 * 직원 모드 공통 하단 네비게이션
 * @param {'home'|'contract'|'alarm'|'my'|'detail'} active — detail: 배차 상세 등에서 탭 모두 비활성(그레이)
 */
export default function StaffBottomNav({ navigate, active = 'home' }) {
  const tabs = [
  ...
  return (
    <nav className="sticky bottom-0 z-20 shrink-0 bg-white border-t border-gray-100 flex shadow-[0_-1px_8px_rgba(0,0,0,0.06)]">
      {tabs.map((t) => {
        const isActive = active === t.id;
        const onDetail = active === 'detail';
        return (
          <button
            key={t.id}
            type="button"
            ...
            className={`flex-1 flex flex-col items-center py-2.5 gap-0.5 text-[11px] transition-colors duration-200 ease-out
              ${isActive && !onDetail ? 'text-olympos-blue font-bold' : 'text-gray-400 font-medium'}
              ${onDetail ? 'opacity-95' : ''}
              hover:text-gray-500 cursor-pointer active:opacity-90`}
          >
            <svg
              className={`w-5 h-5 transition-colors duration-200 ease-out ${
                isActive && !onDetail ? 'text-olympos-blue' : 'text-gray-400'
              }`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              strokeWidth={isActive && !onDetail ? 2.25 : 1.65}
            >
